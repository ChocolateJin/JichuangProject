...
set rc [catch {
  open_project {SOC_2_times.prj}
  import_device eagle_s20.db -package EG4S20BG256
  import_db {../syn_1/SOC_2_times_gate.db}
  place
  export_db {SOC_2_times_place.db}
} RESULT]
if {$rc} {
...
set rc [catch {
  route
  report_area -io_info -file SOC_2_times_phy.area
  export_db {SOC_2_times_pr.db}
} RESULT]
if {$rc} {
...
set rc [catch {
  export_bid SOC_2_times_inst.bid
  bitgen -bit "SOC_2_times.bit"
} RESULT]
if {$rc} {
  step_error bitgen
  return -code error $RESULT
