...
set rc [catch {
  ...
  export_db {SOC_2_times_elaborate.db}
} RESULT]
if {$rc} {
...
set rc [catch {
  read_adc ../../src/pins.adc
  optimize_rtl
  report_area -file SOC_2_times_rtl.area
  export_db {SOC_2_times_rtl.db}
} RESULT]
if {$rc} {
  step_error opt_rtl
  return -code error $RESULT
...
set rc [catch {
  optimize_gate -maparea SOC_2_times_gate.area
  legalize_phy_inst
  export_db {SOC_2_times_gate.db}
} RESULT]
if {$rc} {
  step_error opt_gate
  return -code error $RESULT
